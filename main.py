    ...
    if not is_genesis:
        ...
        if event_results:
            ...
            for unusedEvent in database.get_unused_events(from_star_log=next_star_log['hash']):
                unused_events.append(unusedEvent['key'])
            used_inputs = []
            used_outputs = []
            events = []
...
def sync(params=None):
    ...
    all_results = []
    last_count = util.maximumStarLogSize()
    offset = 0
    while util.maximumStarLogSize() == last_count:
        results = get_request(STAR_LOGS_URL, {'since_time': latest_time, 'limit': util.maximumStarLogSize(), 'offset': offset})
        if results is None:
            last_count = 0
        else:
